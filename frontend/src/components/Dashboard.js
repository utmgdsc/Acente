		...
		axios({
			...
			.then(async function (response) {
				...
				if (response.status === 200) {
					...
					const {
						strongWords: resStrongWords,
						weakWords: resWeakWorks,
						recentSentences: resRecent,
					} = result.data;
					var recentSentences = [];
		...
		<VStack height="86vh" justifyContent="center" spacing="20px">
			...
			>
				<Box
					...
					padding="20px"
				>
					<Center fontSize="3xl" color="gray" justifyContent="left">
						{formatMessage({ id: "dashboard" })}
					</Center>
					<UnorderedList spacing="10px">
						<ListItem>
							Your pronounciation has improved by 10% from last
							week!
						</ListItem>
						<ListItem>
							You set a new record for most sentences practiced,
							way to go!
						</ListItem>
					</UnorderedList>
				</Box>
				...
				>
					...
					<UnorderedList spacing="10px">
						{recentSentences.map((sentence) => (
							<ListItem key={sentence}>{sentence}</ListItem>
						))}
					</UnorderedList>
				</Box>
			</HStack>
